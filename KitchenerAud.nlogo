...
to setup ; linked with setup button on interface
  ...
  ask n-of number patches with [pcolor = white]  ; makes turtles sprout on white patches only
  [
    sprout 1 [
      ...
      ;ask turtles [face one-of turtles with [shape = "square"]]
...
to setup-patches 
  ...
  ; bus stops (source of turtles)
  ask patches with [pycor >= 556 and pycor <= 566 and pxcor >= 20 and pxcor <= -30]
  [set pcolor blue]  

  ; bus stops (source of turtles)
  ask patches with [pycor >= 162 and pycor <= 172 and pxcor >= 693 and pxcor <= 703]
  [set pcolor blue] 
 
end

to draw-exit1
  ;target points in the middle of the aud
  set goal-x 165
  set goal-y 361
...
;  ask turtles with [pxcor > 0] [face one-of turtles with [shape = "square"]]
